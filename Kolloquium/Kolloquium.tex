...
\section{Konzeption}
    \note[enumerate]{
      \item Konzeption
      \item Was musste alles entwickelt werden?
    }
  ...
  \begin{frame}{Was wurde entwickelt?}
    \begin{itemize}[<+- | alert@+>]
      \item<+- | alert@+-+(1)>
        ...
        \begin{uncoverenv}<+-+>%
          \begin{tikzpicture}[overlay, remember picture, railroad]%
            \node[anchor=west, yshift=-1em] at (page cs:-.35,0) (tokenizerMacro) {%
      ...
      \item<+- | alert@+-+(1)>
        ...
        \begin{uncoverenv}<+-+>%
          \begin{tikzpicture}[overlay, remember picture, railroad]%
            \node[anchor=west, yshift=-1em] at (page cs:-.35,0) (parserMacro) {%
      ...
      \item<+- | alert@+-+(1)>
        ...
        \begin{uncoverenv}<+-+>%
          \begin{tikzpicture}[overlay, remember picture]%
            \node[coordinate] at (page cs:-0.35,0) (analyserMacroHelper);
  ...
  \begin{frame}{AST Klassenhierarchie}
    ...
    \end{uncoverenv}%
        ...
        \note[item]<.>{
          Für meinen Geschmack helfen die Diagramme nicht genug, sind aber die beste Möglichkeit anderen Menschen klar zu machen, was für Abhängigkeiten zwischen den Elementen existieren.
        }
  ...
  \begin{frame}{AST Klassen}
    ...
    \end{tikzpicture}%
        \note[item]{
          Die restlichen Abhängigkeiten der AST Klassen.
        }
        \note[item]{
        ...
        \note[item]{
          Eine \textbf{Funktion} hat ein \textbf{Scope}. Ein \textbf{Loop} hat ein \textbf{Scope} und eine \textbf{Condition} und eine Condition kann aus \textbf{Operatoren}, \textbf{Funktionsaufrufen} usw. bestehen.
        }
  \end{frame}
  ...
  \begin{frame}{Interpreter Klassen}
    \begin{uncoverenv}<+-+>%
      ...
      \end{tikzpicture}%
          ...
          \note[item]<.>{
            Der Stack bildet -- mit Instanzen von sich selber -- einen Stack.\ldots
          }
    \end{uncoverenv}
    \begin{uncoverenv}<+-+(1)>%
      \begin{tikzpicture}[remember picture,overlay,every node/.style={anchor=center}]%
      ...
      \end{tikzpicture}%
          ...
          \note[item]<.>{
            Der Stack wird durch die drei Stack Instanzen (\textbf{1, 2 und 3}) gebildet.
          }
          \note[item]<.>{
      ...
      \end{uncoverenv}%
          \note[item]<.>{
            Denn dafür wisst ihr noch zu wenig. Nehmt es erst einmal so hin.
          }
    \end{uncoverenv}
    ...
    \begin{uncoverenv}<+-+>%
      ...
      \end{tikzpicture}%
          ...
          \note[item]<.>{
            Im Funktionsaufrufe auszuführen -- die nicht zu selber definierten Funktionen führen -- nutzt der Interpreter den \textbf{CommandProvider}.
          }
    \end{uncoverenv}
  \end{frame}
