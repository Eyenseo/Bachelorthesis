  ...
  \begin{frame}{Was wird entwickelt werden?}
    \begin{itemize}[<+- | alert@+>]
      \item<+- | alert@+-+(1)>
        ...
        \begin{uncoverenv}<+-+>%
          \begin{tikzpicture}[overlay, remember picture,railroad]%
            \node[right=0of tokenizer, anchor=north west, yshift=1.45em] (tokenizerMacro) {%
      ...
      \item<+- | alert@+-+(1)>
        ...
        \begin{uncoverenv}<+-+>%
          \begin{tikzpicture}[overlay, remember picture, railroad]%
            \node[anchor=north west, yshift=3.45em] at ($(tokenizerMacro.west)$ |- parser) (parserMacro) {%
      ...
      \item<+- | alert@+-+(1)>
        ...
        \begin{uncoverenv}<+-+>%
          \begin{tikzpicture}[overlay, remember picture]%
            \node[anchor=north west, yshift=1em] at ($(tokenizerMacro.west)$ |- analyser) (analyserMacro) {%
              \myMIn$def fun()\{ break; \}$
            };%
          \end{tikzpicture}%
        \end{uncoverenv}
            \note[item]<.>{
              Ein Beispiel wäre die Funktion \myMin{fun}. Der AST kann den Code darstellen. Eine Funktionsdefinition, die in dem Funktionsscope ein break Element hat.
            }
            \note[item]<.>{
