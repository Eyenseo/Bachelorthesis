...
\section{Realisierung}
  ...
  \begin{frame}{Interpreter/Stack}
    ...
    \end{uncoverenv}%
        ...
        \note[item]<.>{
          Am Anfang gibt es eine Root Node.\ldots
        }
    ...
    \end{uncoverenv}%
        \note[item]<.>{
          Als erstes definiert der Interpreter die Funktionen eines Scopes, bevor er anfängt diese zu interpretieren.
        }
        \note[item]<.>{
    ...
    \end{uncoverenv}%
        \note[item]<.>{
          Der der String ``Eins Leerzeichen'' wird bar zugewiesen.
        }
    ...
    \end{uncoverenv}%
        \note[item]<.>{
          Danach wird \textbf{fun} aufgerufen, wodurch wieder eine Stack Instanz zu dem Stack hinzugefügt wird.
        }
        \note[item]<.>{
    ...
    \begin{uncoverenv}<+>%
      \begin{tikzpicture}[overlay, remember picture]%
        \node[anchor=center] at (page cs:0,0) (finish-sleepy) {\includegraphics[width=.4\linewidth]{img/sleepy_cat}};
      \end{tikzpicture}%
    \end{uncoverenv}%
        \note[item]<.>{
