...
\label{sec:Anforderungsanalyse}
  ...
  \label{ssec:Diskussion des Problemfeldes}
    ...
    \label{sssec:Das vorhandene System}
      Das vorhandene Softwaresystem ist eine C++ Anwendung, die mit Hilfe von Modulen ihre Funktionalität erweitern kann. Ein Teil des zentralen Herzstückes ist eine Implementation des Command-Patterns, siehe \autoref{fig:commanduml}.
  ...
  \label{ssec:Parser}
    ...
    \label{sssec:Definition}
      ...
      \paragraph{Funktion}
        ...
        Das Ergebnis, von dem Beispiel (\myMIn$def fun(foo) {$), ist in \autoref{fig:exastfundef} zu sehen, \myMinin{ast::Define} (Zeile 1) enthält die \myMinin{ast::Function} (Zeile 3) mit ihren Parametern (Zeile 6 ff.) und dem Funktionsscope (Zeile 9).
        ...
        \begin{paracol}{2}
          ...
          \begin{myCodeEnv}
            ...
            \end{myInvBox}
            \caption*{Aktuelle TokenList}
            \label{fig:exastfundefli}
          \end{myCodeEnv}
        \end{paracol}

      \paragraph{Variable}
        Wenn das geparsede \myMinin{Token} \myRIn$var$ war, wird die \myMinin{TokenList} an die Funktion für \myMinin{ast::Variable} übergeben. Diese erwartet, als aktuelles \myMinin{Token} einen Bezeichner ist. Es ist also wichtig, dass zuerst probiert wird, eine \myMinin{ast::Funktion} zu parsen und erst im Anschluss eine \myMinin{ast::Variable}.

        Vermutlich, entgegen die Erwartungshaltung des Lesers, wird hier nicht die Zuweisung behandelt. Dies geschieht bei der \myMinin{ast::Operator} Funktion, im Verlauf der Erklärung, wie Operatoren geparsed werden, sollte es offensichtlich werden, wieso dies so ist.
    ...
    \label{sssec:do-while}
      ...
      \begin{paracol}{2}
        ...
        \begin{myCodeEnv}
          \centering
          \begin{myInvBox}[width=.9\linewidth]
            \lstinputlisting[firstline=7,lastline=17]{code/extok.txt}
          \end{myInvBox}
          \caption*{Aktuelle TokenList}
          \label{fig:exastdowhileli}
        \end{myCodeEnv}
      \end{paracol}
    ...
    \label{sssec:Operator und Condition}
      Der Unterschied zwischen dem Parsen, von einem Operator und einer Condition ist, dass Operatoren geparsed werden, wenn der linke Teil des Operators (im Fall, dass es sich um einen binären Operator handelt), schon geparsed ist. Die Logik der \myMinin{ast::Operator} parse Methode muss dies berücksichtigen, die \myMinin{ast::Condition} parse Methode nutzt intern die \myMinin{ast::Operator} Methode um solche Fälle zu unterstützen. Abgesehen von diesem Unterschied, können die beiden Methoden gleich behandelt werden.
      ...
      \paragraph{Binäroperator}
        In Zeile 3, des Beispieles (\myMIn$foo = foo + 1.1;$), sieht man den Fall, in dem der linke Operand von dem Operator schon geparsed ist. \myMinin{foo} wurde als Variable geparsed, da dieser Parser ohne Vorausschauen implementiert wurde. Als nächstes Token ist \myTIn$=$ in der Liste, was das parsen von einem binären Operator indiziert. Die \myMinin{ast::Operator} Methode erwartet, dass  -- in dem Fall, dass es sich um einen binären Operatoren handelt -- das zuletzt geparsede Element ein Werterzeuger ist (\autoref{fig:vprsyntax}). Dieses wird als linkes Element des \myMinin{ast::Operator} gesetzt. Als nächstes erwartet die Methode, dass das nächste \myMinin{Token} auch ein Werterzeuger ist.
      ...
      \paragraph{Unäroperator}
        ...
        Die \autoref{fig:exastdowhilecon} zeigt die geparsede \myMinin{ast::Condition} aus Zeile 4, des Beispiels (\myMIn$} while(!foo);$).
        \begin{paracol}{2}
          \begin{myCodeEnv}
            \centering
            \begin{myInvBox}[width=.9\linewidth]
    ...
    \label{sssec:Literals}
      ...
      \begin{paracol}{2}
        ...
        \begin{myCodeEnv}
          ...
          \end{myInvBox}
          \caption*{Aktuelle TokenList}
          \label{fig:exastdowhilescopedoubleli}
        \end{myCodeEnv}
      \end{paracol}
