...
\pagebreak
  ...
  \label{ssec:Danksagung}
    Ich möchte mich bei meinen Eltern bedanken, die mich immer unterstützt haben und dafür sorgten, dass ich nicht in der Sonderschule ende! Meine Freunden und allen Menschen, die mich zu diesem Studium geführt haben, verdienen auch mehr als meinen Dank -- sonst wäre ich wohl Bauer geworden\ldots Ein großes Dankeschön gilt auch meinen Professoren und meinem Betreuer bei der P3-Group, die mich diese Arbeit schreiben ließen, die aus unterschiedlichen Gründen ungewöhnlich ist.
    \begin{figure}[H]
      \centering
      \begin{tikzpicture}[%
        spy using outlines={%
...
\label{sec:Einleitung}
  ...
  \label{ssec:Vorhandene Lösungen}
    ...
    ChaiScript bietet von Hause aus viele Funktionalitäten, die weit über den Bedarf für eine Makrosprache hinausgehen. Dies ist auch der Grund, wieso sich gegen die Nutzung von ChaiScript entschieden wurde. Da beliebige Funktionen aufgerufen werden können, bietet dies eine unangenehm große Fehlerquelle, wenn diese Funktionen den Zustand der Applikation verändern, ohne das dies Rückgängig gemacht werden können.
\myQuestion{Reicht das? Ich kenne mich zu wenig mit dem Projekt aus, um einen Kompletten Vergleich zu schreiben.\\Es ist außerdem langsam zu kompilieren (1min für den Header, was explodieren kann...), nutzt virtuelle Methoden und Exceptions für den Kontrollfluss \emph{ihhh}.}
  ...
  \label{ssec:Ziele der Arbeit}
    ...
    \begin{itemize}
      \item auf keinem festen \emph{Application Programming Interface (API)} aufbaut.
        ...
        } erweitert werden kann. Eines dieser Module wird das Makrosystem sein, welches in dieser Arbeit entwickelt wird. Durch diese Modularität, gibt es kein festes Interface.
...
\myQuestion{Ist das so verständlich?}
...
\label{sec:Konzeption}
  ...
  \label{ssec:Syntax}
    ...
    \label{sssec:Syntax Grundlagen}
      Bezeichner müssen dem regulären Ausdruck (Regex) aus \autoref{fig:idsyntax} entsprechen.

      Das heißt, dass Bezeichner nur aus kleinen Buchstaben, Nummern und Unterstrichen bestehen können und am Anfang einen Buchstaben haben müssen. Grund für diese drastische Einschränkung ist, dass der Code einheitlich aussehen soll (die erste Regel, was Bezeichnungen/Formatierung angeht ist, dass man sich an dem orientiert, was schon existiert). Um dies besser garantieren zu können, wurde die CamelCase Schreibweise von vornherein ausgeschlossen. Außerdem sind Bezeichner, die einem Keyword
      (%
        \myMIn$break$,
        \myMIn$continue$,
        \myMIn$def$,
...
\myQuestion{Ich habe die regex Erklärung hier herein geschrieben -- war das sinnvoll oder soll ich das lieber weglassen?}
      Regex:
      \begin{itemize}
        \item \myTIn$.$ kann jedes Zeichen sein.
        \item \myTIn$*$ bedeutet null, \myTIn$+$ bedeutet ein, oder mehr vom vorausgehenden Zeichen.
        \item \myTIn$?$ bedeutet null oder ein Mal, in Verbindung mit \myTIn$*?$ wird beim ersten validen Regex abgebrochen.
        \item \myTIn$|$ bedeutet oder.
        \item \myTIn$[]$ umfasst eine Sammlung von Zeichen, bei dem alle Zeichen einzeln verglichen werden. \myTIn$[^]$ entspricht allen Zeichen, die nicht genannt sind. In einer Sammlung kann \myTIn$-$ genutzt werden, um eine Reihe von Zeichen zu bestimmen.
        \item \myTIn$\s$ sind Whitespace Zeichen.
        \item \myTIn$\d$ sind Nummernzeichen.
        \item \myMin{\} escaped das folgende Zeichen.
      \end{itemize}
  ...
  \label{ssec:Grundarchitektur}
    ...
    \label{sssec:Token und Parser Paket}
      ...
      \paragraph{Tokenizer}
        ...
        }). Der \myMinin{Tokenizer} ist nicht für die Umwandlung (Lexen) verantwortlich. Beim Lexen wird z.B. aus dem Token \myMinin{"1.01"} der Double \myMinin{1.01}. Das Lexen übernimmt der Parser.
    ...
    \label{sssec:Interpreter Paket}
      ...
      \paragraph{Stack}
        Während der Ausführung des Makros durch den \myMinin{Interpreter} übernimmt der \myMinin{Stack} die Verwaltung der definierten Funktionen und Variablen sowie deren Werte und repräsentiert demzufolge die \myMinin{ast::Scope}s. Wird ein neues \myMinin{ast::Scope} in dem \myMinin{AST} geöffnet, wird ein neuer \myMinin{Stack} erzeugt, der als vorherigen \myMinin{Stack} den aktiven \myMinin{Stack} bekommt. Durch diese Verkettung wird eine verkettete Liste aufgebaut, bei der man nur das zuletzt hinzugefügte Objekt entfernen muss, um ein \myMinin{ast::Scope} zu beenden, siehe \autoref{fig:stackex}. In \autoref{sssec:Stack Architektur} wird die Architektur weiter erläutert.
        \begin{figure}[H]
          \centering
...
\myQuestion{Ich habe bis hier keine Programmiersprachen genannt -- ist das in Ordnung das ich hier das Kapitel bringe?}
      Da das vorhandene System in C++ geschrieben ist, ist es größtenteils unnötig, über andere Programmiersprachen nachzudenken.
      ...
      \begin{itemize}
        \item dependency circle\\
          ...
          Da das \myMinin{Scope} Instanzen von allen \myMinin{AST} Klassen aufnehmen kann, und einige Klassen wiederum ein \myMinin{Scope} besitzen, kommt es hier zu einem dependency circle. Da das \myMinin{Scope} eine der meist benutzten Klassen ist, liegt die Auflösung des dependency circles bei den anderen Klassen.
        \item plain old data\\
...
\label{sec:Exemplarische Realisierung}
  ...
  \begin{paracol}{2}[\subsection{Tokenizer}]
    ...
    \label{ssec:Tokenizer}
      ...
      \paragraph{Dezimalzahl}
        Wenn das Zeichen eine Zahl ist, probiert der \myMinin{Tokenizer} den Regex \myRIn$\d*\.\d+$ auf das aktuelle und die folgenden Zeichen anzuwenden -- siehe \autoref{fig:extok} Zeile 13. Integer werden als normale Token geparst. (Siehe \ref{enum:lexnumber} für Verbesserungen.)
  ...
  \label{ssec:Parser}
    ...
    \label{sssec:Definition parsen}
      ...
      \paragraph{Funktion}
        ...
        Wenn Fehler beim Parsen auftreten -- zum Beispiel, dass kein Bezeichner oder das \myMinin{ast::Scope} fehlt -- schmeißt die Methode eine Exception, die bis zu der \myMinin{static} globalen Methode nach oben wandert. Das Parsen der Funktion ist von einem \myMinin{try-catch} Block umschlossen. Sollte also eine Exception geworfen werden, wird diese gefangen, und mit dem Dateinamen, Zeile, Spalte und Quellcode der Zeile erweitert, um dann wieder geworfen zu werden.
    ...
    \label{sssec:Literals parsen}
      Um Literals zu parsen bzw. zu erkennen, wendet der \myMinin{Parser} für Strings \myRIn$".*"$, Integer \myRIn$\d+$, Doubles \myRIn$\d*\.\d+$ und Booleans \myRIn$true|false$ als Regex an. Für den String reicht in diesem Fall der Regex aus, weil der \myMinin{Tokenizer} den String als ein \myMinin{Token} produziert hat.

      \begin{lrbox}{\myMinSavBox}
        \myMinin{"t\\tt"}$\rightarrow$\myMinin{"t\tt"}
      \end{lrbox}

      Um die nummerischen Tokens zu Werten zu konvertieren (lexen), nutzt der \myMinin{Parser} die C++ Funktionen \myMinin{std::stod(...)} und \myMinin{std::stoi(...)}. Für die Booleanwerte reicht die Überprüfung, ob das \myMinin{Token} ein Boolean darstellt. Danach wird der String kopiert und anschließend werden alle escapeden Zeichen unescaped (\myMinin{"t\tt"}$\rightarrow$\myMinin{"t  t"}\footnote{
    ...
    \label{sssec:Return parsen}
      Return ist leicht zu parsen, weil es nur einen Werterzeuger nach dem Keyword \myRIn$return$ erwartet.
    ...
    \label{sssec:Analyser}
      ...
      Ein Fehler den der \myMinin{Analyser} nicht prüfen kann bedeutet, dass eine Funktion nicht existiert. Da der \myMinin{Interpreter} auf den \myMinin{CommandProvider} zugreift, um Funktionen zu finden, und diese Funktionen durchaus durch ein Makro registriert werden können, ist ein solcher Fehler erst zur Laufzeit zu finden.
  ...
  \label{ssec:Interpreter}
    Der \myMinin{Interpreter} durchläuft den AST wie der \myMinin{Analyser} recursive decent. Anstatt Signale bei den AST Elementen auszuführen, setzt der \myMinin{Interpreter} die Elemente um. Der \myMinin{Interpreter} ist durch die Nutzung von \myMinin{Stack} und \myMinin{OperatorProvider} eine der kleinsten Klassen -- abgesehen von den Klassen aus dem \myMinin{pod} Paket.
    ...
    \label{sssec:Funktionsaufruf}
      Bei dem Funktionsaufruf (\myMinin{return fun(foo:foo);}), fragt der \myMinin{Interpreter} als erstes den \myMinin{Stack} nach einer Referenz eines \myMinin{ast::Function} Objektes, welches dem Funktionsaufruf gleicht. Es muss also eine Funktion definiert worden sein, die dem Bezeichner \myRIn$fun$ gleicht und einen Parameter namens \myRIn$foo$ besitzt. Im Anschluss erstellt der \myMinin{Interpreter} einen neuen \myMinin{Stack}, der den root \myMinin{Stack} als Pointer hat -- siehe \autoref{fig:intstack}. Wenn der neue \myMinin{Stack} den \myMinin{main()} \myMinin{Stack} als Pointer hätte, könnte der neue \myMinin{Stack} direkt auf alle Variablen aus der \myMinin{main()} Methode zugreifen. Dem neuen \myMinin{Stack} wird dann ein Alias mit dem Namen \myMinin{foo} hinzugefügt, der auf die \myMinin{foo} Variable aus dem \myMinin{main()} \myMinin{Stack} zeigt. Dies ist in \autoref{fig:int8} zu sehen. Nachdem der \myMinin{Stack} vorbereitet ist, wird das \myMinin{ast::Scope} der Funktionsdeklaration interpretiert.
    ...
    \label{sssec:Do-While}
      Ein \myMinin{ast::DoWhile} wird interpretiert, indem das \myMinin{ast::Scope} ausgeführt wird und im Anschluss die \myMinin{ast::Condition} ausgewertet wird. Sollte das Ergebnis \myMinin{true} sein, wird das \myMinin{ast::Scope} wieder ausgeführt und am Ende wieder die \myMinin{ast::Condition} geprüft.

      Während der Ausführung ist es möglich, dass \myMinin{return} oder z.B. \myMinin{break} aufgerufen wird. Wenn eines dieser beiden Elemente interpretiert wird, wird ein Flag gesetzt, das der \myMinin{DoWhile} Methode mitteilt, ob der Loop unterbrochen wurde. Diese Logik findet sich in allen \myMinin{ast::Loop} und der \myMinin{ast::Scope} Methode wieder.
    ...
    \label{sssec:Operator}
      Um \myMinin{ast::Operator} Objekte zu interpretieren, löst der \myMinin{Interpreter} die Werterzeuger so weit auf, dass zwei \myMinin{any} Werte hat. Diese Werte werden, mit dem Operator als \myMinin{enum}, an den \myMinin{OperatorProvider} zum Evaluieren gegeben.
      ...
      Der Assignmentoperator wird nicht von dem \myMinin{OperatorProvider} angeboten, weil dieser abhängig von dem \myMinin{Stack} State ist. Wenn die Variable ein Alias ist (wie in dem Fall des Beispiels -- siehe \autoref{fig:int8}), muss dieser gelöscht werden und durch eine `echte' Variable ersetzt werden, bevor der Wert verändert werden kann.
...
\label{sec:Evaluation}
  Das Ergebnis dieser Arbeit erfüllt alle Anforderungen wie Ziele, die diese Arbeit sich am Anfang gestellt hat zu lösen. In den folgenden zwei Kapiteln wird darauf eingegangen, was bei der Architektur und Implementation zu kurz gekommen ist, was verbessert werden kann oder gut gelöst wurde.
  ...
  \label{ssec:Architektur}
    Die entwickelte Architektur ist, meiner Ansicht nach, gut und auch für Erweiterungen gut platziert.

    Das die Architektur gut erweiterbar ist, liegt aus meiner Sicht daran, dass es sich bei dem AST um PODs handelt. Damit ist die Logik von den Daten getrennt, was dafür sorgt, dass durch Veränderungen, an z.B. dem \myMinin{Parser} der \myMinin{Analyser} nicht betroffen ist und so keine Fehler bei diesem eingeführt werden können.

    Ein weiterer positiver Punkt bei der Entwicklung der Klassen ist, dass \myMinin{variant} genutzt wurde. Durch die Nutzung von \myMinin{variant} sind Fehler, die durch \myMinin{union}s entstehen können minimiert worden und die Größe der Objekte hat nicht zu sehr darunter gelitten. Eine Alternative zu \myMinin{union}s und \myMinin{variant} wären \myMinin{virtual} Methoden -- diese sind allerdings langsamer.
  ...
  \label{ssec:Implementation}
    Die Implementierung ist ausreichend um Automatisierungen vorzunehmen, allerdings ist sie nicht vollständig genug, um sie für alles ohne Bedenken nutzen zu können.

    Das Abhanden sein von Arrays ist eine klare Einschränkung, was die Kompatibilität angeht. Das es keine Arrays gibt beruht jedoch nur darauf, dass trotz eines straffen Zeitplans nicht genug Zeit vorhanden war, um auch diese zu implementieren. Da dies von vorn herein klar war, wurden die Arrays auch in dem \myNamedRef{ssec:Syntax} ausgelassen.

    Einer der schwerwiegendsten Punkte ist, dass die Zeit nicht für ausgiebigere Tests gereicht hat und auch keine Performance Tests ausgeführt werden konnten. Die Testabdeckung ist somit nur ausreichend und definitiv nicht gut, wenn man dem Test-Driven-Development folgen will.

    Ein, aus meiner Sicht, positiver Punkt der Implementierung ist, dass C++14 und Klassen, die in dem kommenden C++17 Standard enthalten sind, genutzt wurden um die Architektur umzusetzen. Dies hat nicht nur für eine klare bzw. saubere Implementation gesorgt, sondern sorgt auch dafür, dass Fehler die durch z.B. die Nutzung von \myMinin{union}s entstanden wären, ausgeschlossen sind.
...
\label{sec:Zusammenfassung und Ausblick}
  Alles in Allem, bin ich sehr zufrieden mit dem Ergebnis. Mit einer Woche mehr für die Implementation, hätte man auch noch die Arrays implementieren können, aber auch ohne diese, ist
  das Ergebnis dieser Arbeit gut einsetzbar.

  Das folgende Kapitel enthält Punkte, die während der Implementierung aufgefallen sind und bei bedarf bzw. Zeit umgesetzt werden sollten.
  ...
  \label{ssec:Ausblick}
    \begin{enumerate}[ref=\autoref{ssec:Ausblick}: Punkt \arabic*]
      ...
      \item Verkettete \myMinin{Stack} Liste in ein Array umwandeln\label{enum:linkedlist}\\
        Wenn sich herausstellt, dass die verkettete Liste von dem \myMinin{Stack} zu langsam ist, kann der Interpreter ein Array nutzen, um die \myMinin{Stack}s zu speichern und die \myMinin{Stack}s nutzen, anstelle eines Pointers einen Offset, von dem sie aus die anderen \myMinin{Stack} fragen können. Der Vorteil von Arrays gegen verkettete Listen ist die wesentlich höhere Cache-Locality.
